...
begin
  ...
  loop
    ...
    if v_amount <= 0 then
      -- A zero-value package cannot produce a wallet transaction because the
      -- ledger explicitly rejects zero amounts. Advance it without fabricating
      -- a credit so it cannot block other packages.
      update public.user_products
      set days_collected = days_collected + 1,
          last_collected_on = p_collection_date,
          next_collection_on = p_collection_date + 1,
    ...
    v_reference := 'daily-income:' || v_package.id::text || ':' || p_collection_date::text;

    -- The unique reference makes retries safe. Only a newly inserted ledger row
    -- is allowed to change the wallet and package counters.
    insert into public.wallet_transactions (
      ...
      v_reference,
      jsonb_build_object('user_product_id', v_package.id, 'collection_date', p_collection_date)
    )
